...
export default defineComponent({
  ...
  setup() {
    ...
    async function getData() {
      const result = (await getAllSinger()) as ResponseBody;
    ...
    function uploadUrl(id) {
      return attachApiUrl(`/singer/avatar/update?id=${id}`);
    }
    ...
    async function addsinger() {
      ...
      const result = (await setSinger({ name, sex, birth, location, introduction })) as ResponseBody;
    ...
    async function saveEdit() {
      try {
        ...
        const result = (await updateSingerMsg({ id, name, sex, birth, location, introduction })) as ResponseBody;
        (proxy as any).$message({
          message: result.message,
          type: result.type,
        });
    ...
    async function confirm() {
      const result = (await deleteSinger(idx.value)) as ResponseBody;
      (proxy as any).$message({
        message: result.message,
        type: result.type,
      });
