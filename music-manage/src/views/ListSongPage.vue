...
export default defineComponent({
  ...
  setup() {
    ...
    async function confirm() {
      try {
        const songListId = proxy.$route.query.id as string | number;
        const result = (await deleteListSong(songListId, idx.value)) as ResponseBody;
        (proxy as any).$message({
          message: result.message,
          type: result.type,
        });
        ...
        const body = err?.data as ResponseBody | undefined;
        const msg = body?.message || err?.statusText || "删除失败，请稍后重试";
        (proxy as any).$message({ message: String(msg), type: "error" });
      } finally {
    ...
    async function deleteAll() {
      ...
      for (const item of rows) {
        try {
          const result = (await deleteListSong(songListId, item.id)) as ResponseBody;
      ...
      if (rows.length > 0 && failed === 0) {
        (proxy as any).$message({ message: "批量删除成功", type: "success" });
      } else if (rows.length > 0) {
        (proxy as any).$message({ message: `批量删除完成，有 ${failed} 条未成功`, type: "warning" });
      }
      getData();
    }
