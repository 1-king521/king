...
const smsCodeFallback: ApiResponse<null> = {
  code: 500,
  success: false,
...
export async function fetchSendLoginSmsCode(phone: string): Promise<ApiResponse> {
  try {
    return await api({
      method: "post",
    ...
    const message =
      error?.data?.message || error?.response?.data?.message || smsCodeFallback.message;
    return {
      ...smsCodeFallback,
      message,
    };
...
const smsSignInFallback: ApiResponse<null> = {
  code: 500,
  success: false,
  message: "登录失败，请稍后重试",
  data: null,
...
export async function fetchSmsSignIn(payload: SmsSignInPayload): Promise<ApiResponse> {
  try {
    return await api({
      method: "post",
      url: "login",
    ...
    const message =
      error?.data?.message || error?.response?.data?.message || smsSignInFallback.message;
    return {
      ...smsSignInFallback,
      message,
    };
