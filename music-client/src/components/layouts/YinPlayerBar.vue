<template>
  <div class="playbar" :class="{ 'is-collapsed': !toggle }">
    ...
    <el-slider
      ...
      @update:model-value="onProgressSliderDrag"
...
/** 拖进度条时拿到真实时长（store 未同步时用 audio.duration） */
const yinAudioRef = ref<InstanceType<typeof YinAudio> | null>(null);
...
/** 与 v-model 同步：拖动时一定会触发 update:model-value；仅依赖 @input 时 watch 会回写进度导致「拖不动」 */
function onProgressSliderDrag() {
  isDragging.value = true;
}

function playbackDurationSec(): number {
  const fromStore = duration.value;
  if (Number.isFinite(fromStore) && fromStore > 0) return fromStore;
  const audio = yinAudioRef.value?.getAudioElement?.() ?? null;
  const d = audio?.duration;
  if (audio && Number.isFinite(d) && d > 0) {
    songStore.setDuration(d);
    return d;
  }
  return 0;
}

// 拖动结束：用滑块「最终值」算目标秒数（@change 触发时本地 nowTime 可能尚未与 v-model 同步，会误算成 0 导致像重播）
function onSliderChange(value: number | number[] | undefined) {
  const total = playbackDurationSec();
  if (!Number.isFinite(total) || total <= 0) {
    isDragging.value = false;
    return;
  }
  const pct = Array.isArray(value) ? value[0] : (value ?? nowTime.value);
  const ratio = Math.min(100, Math.max(0, Number(pct))) * 0.01;
  const target = total * ratio;
  if (!Number.isFinite(target)) {
    isDragging.value = false;
    return;
  }
  nowTime.value = Number(pct);
  songStore.setChangeTime(target);
  isDragging.value = false;
}
...
.playbar__toggle .playbar__icon-btn {
  pointer-events: auto;
}

/* 进度条必须在主内容之上；折叠钮曾压在滑条之上会挡住左侧轨道 */
.playbar__progress {
  position: absolute;
  left: 0;
  right: 0;
...
.playbar__progress :deep(.el-slider__runway) {
  height: 10px;
  margin: 6px 0;
  touch-action: none;
}

.playbar__progress :deep(.el-slider__bar) {
  height: 10px;
}

.playbar__progress :deep(.el-slider__button-wrapper) {
  top: -7px;
}
