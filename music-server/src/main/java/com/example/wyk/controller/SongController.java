...
import com.example.wyk.model.request.SongLyricUpdateRequest;
...
public class SongController {
    ...
    /** 仅更新歌词（自动拉取后回写数据库） */
    @PostMapping("/song/lyric/update")
    public R updateSongLyric(@RequestBody SongLyricUpdateRequest body) {
        if (body.getId() == null) {
            return R.error("id不能为空");
        }
        return songService.updateSongLyric(body.getId(), body.getLyric());
    }
    ...
    public R getSongRank() {
        return songService.getSongCollectRank();
    }
}
